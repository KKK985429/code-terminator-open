...
def test_kimi_local_integration_assets_exist() -> None:
    ...
    assert (root / "scripts" / "run_kimi_local_suite.py").is_file()
    assert (root / "scripts" / "run_kimi_worker_contract_local.py").is_file()
    assert (root / "scripts" / "kimi_local_suite_lib.py").is_file()
